...
def compute_pixel_level_metrics(pred, target):
    ...
    tp = np.sum(pred * target)  # true positives
    tn = np.sum((1 - pred) * (1 - target))  # true negatives
    fp = np.sum(pred * (1 - target))  # false positives
    ...
    # IoU for foreground
    iou_fore = tp / (tp + fp + fn + 1e-10)
    # IoU for background
    iou_back = tn / (tn + fp + fn + 1e-10)
    # mean IoU
    miou = (iou_fore + iou_back) / 2

    pred = torch.from_numpy(pred).to(torch.int64)  # 等价于 long
    target = torch.from_numpy(target).to(torch.int64)
    dice = dice_coeff_multi_class(pred, target, n_classes=2)

    return [acc, miou, recall, precision, F1, dice]


def dice_coeff_multi_class(pred, target, n_classes):
    """Calculate the mean Dice Coefficient for multi-class data (robust version)."""
    dice_scores = []
    smooth = 1.0
    ...
    for cls in range(n_classes):
        ...
        union = pred_cls.float().sum() + target_cls.float().sum()
...
def compute_hausdorff95(pred, target):
    """Compute the 95% Hausdorff distance between prediction and target"""
    pred_bound = extract_boundary_coords(pred)
    target_bound = extract_boundary_coords(target)

    if len(pred_bound) == 0 or len(target_bound) == 0:
        return 0.0  # 如果某个为空，认为 Hausdorff 距离为 0，或你可以返回 NaN

    d1 = directed_hausdorff(pred_bound, target_bound)[0]
    d2 = directed_hausdorff(target_bound, pred_bound)[0]
    return max(d1, d2)


def extract_boundary_coords(binary_mask):
    """Extract boundary pixels as coordinates from a binary mask"""
...
def gland_accuracy_object_level_val(pred, gt):
...
def instance_level_iou_only(pred, gt):
    """
    Compute instance-level IoU between predicted and groundtruth masks.
    Returns:
        iou_g, iou_s, gt_objs_area, pred_objs_area
    """
    ...
    pred_labeled = morph.label(pred, connectivity=2)
    pred_objs_area = np.sum(pred_labeled > 0)

    gt_labeled = morph.label(gt, connectivity=2)
    gt_labeled = morph.remove_small_objects(gt_labeled, 3)
    gt_labeled = morph.label(gt_labeled, connectivity=2)
    gt_objs_area = np.sum(gt_labeled > 0)
    ...
    iou_g = 0.0
    for i in range(1, Ng + 1):
        gt_i = (gt_labeled == i).astype(np.uint8)
        overlap_parts = gt_i * pred_labeled
        obj_no = np.unique(overlap_parts)
        obj_no = obj_no[obj_no != 0]
        ...
        if obj_no.size == 0:
            ...
            obj_areas = [np.sum(overlap_parts == k) for k in obj_no]
            seg_obj = obj_no[np.argmax(obj_areas)]
            pred_i = (pred_labeled == seg_obj).astype(np.uint8)

            overlap_area = np.sum(pred_i * gt_i)
            union_area = np.sum(pred_i) + np.sum(gt_i) - overlap_area
            iou_i = overlap_area / union_area

        iou_g += np.sum(gt_i) * iou_i

    iou_s = 0.0
    for j in range(1, Ns + 1):
        pred_j = (pred_labeled == j).astype(np.uint8)
        overlap_parts = pred_j * gt_labeled
        obj_no = np.unique(overlap_parts)
        obj_no = obj_no[obj_no != 0]
        ...
        if obj_no.size == 0:
            ...
            gt_areas = [np.sum(overlap_parts == k) for k in obj_no]
            gt_obj = obj_no[np.argmax(gt_areas)]
            gt_j = (gt_labeled == gt_obj).astype(np.uint8)

            overlap_area = np.sum(pred_j * gt_j)
            union_area = np.sum(pred_j) + np.sum(gt_j) - overlap_area
            iou_j = overlap_area / union_area

        iou_s += np.sum(pred_j) * iou_j

    return iou_g, iou_s, gt_objs_area, pred_objs_area
...
def split_forward(model, input, size, overlap, outchannel=3):
    ...
    if h0 - size > 0:
        ...
        if tmp.device != input.device:
            tmp = tmp.to(device=input.device, dtype=input.dtype)
